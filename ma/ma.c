...
void dtls_ma_generate_private_key(struct dtls_ma_private_key *privk) {
    ...
    for (i = 0; i < DTLS_MA_N; i++) mpz_init(privk->k[i]);
    ...
    //  pStep = (privk->q - (privk->t-1lu)*privk->sk)/(DTLS_MA_W*privk->r) -
    //  privk->t; //$sk*(t-1)+w*r*(t+pStep)<q$
    // do validity check
    ...
    if (mpz_cmp(b, privk->q) >= 0) {
        dtls_ma_debug(
            "\nError: the parameters DTLS_MA_T_BITS or DTLS_MA_SK_BITS "
            "too big with respect to DTLS_MA_Q_BITS");
        return;
    }
    ...
    mpz_fdiv_q(a, a, b);  // a will be the maximum of random numbers that can be
    // added to t to get privk->p
    mpz_add_ui(b, privk->t, 0xFF);

    if (mpz_cmp(a, b) < 0) {
        dtls_ma_debug(
            "\nError: the parameters DTLS_MA_E_BITS and DTLS_MA_SK_BITS "
            "too big with respect to DTLS_MA_Q_BITS");
        return;
    }
...
struct dtls_ma_public_key *dtls_ma_load_public_key(const char *fname) {
    ...
    mpz_init(pubk->wMin);
    mpz_init(pubk->bias);
    for (i = 0; i < DTLS_MA_M; i++) mpz_init(pubk->Element[i]);
...
void dtls_ma_get_public_key(struct dtls_ma_private_key *privk,
                            struct dtls_ma_public_key *pubk) {
    // struct dtls_ma_public_key* pubk = malloc(sizeof(struct
    // dtls_ma_public_key));
    ...
    for (i = 0; i < DTLS_MA_M; i++) {
        ...
        for (j = 0; j < DTLS_MA_N; j++) {
            mpz_mul_si(
                a, privk->k[j],
                dtls_ma_common[(i + (j + 1) * bias) % DTLS_MA_COMMON_LEN]);
        ...
        if (DTLS_MA_E_SIGN == '-') mpz_mul_si(e, e, -1);
...
void dtls_ma_print_SIS_sample(char *msg, struct dtls_ma_public_key *pubk,
    ...
    int i, j;
    printf(
        "\n\n Copy the expression to "
        "https://develop.wolframcloud.com/objects/"
        "63785264-af0d-49e5-83e4-36465169a55d\n\nFindInstance[{\n");
    for (j = 0; j < DTLS_MA_N; j++) {
        for (i = 0; i < DTLS_MA_M; i++) {
            if (i == DTLS_MA_M - 1)
                printf("(%d)*l%d==%ld, \n",
                       (dtls_ma_common[(i + (j + 1) * mpz_get_ui(pubk->bias)) %
                                       DTLS_MA_COMMON_LEN]),
                       i, mpz_get_si(D->element[j]));
            else
                printf("(%d)*l%d+",
                       (dtls_ma_common[(i + (j + 1) * mpz_get_ui(pubk->bias)) %
                                       DTLS_MA_COMMON_LEN]),
...
void dtls_ma_enc(struct dtls_ma_public_key *pubk, mpz_t v,
    ...
    for (i = 0; i < DTLS_MA_N; i++) {
        ...
        for (j = 0; j < sel; j++) {
            mpz_set_si(
                a,
                dtls_ma_common[(alpha[j] + (i + 1) * mpz_get_ui(pubk->bias)) %
                               DTLS_MA_COMMON_LEN]);
...
int dtls_ma_cipher_out_str(struct dtls_ma_cipher *D, char *buf, int buf_len) {
    ...
    for (i = 0; i < DTLS_MA_N; i++) {
        if (len > buf_len) {
#ifdef DTLS_MA_DEBUG
            printf("buf not long enough: expected at least %d, actual %d", len,
                   buf_len);
#endif
            return -1;
        }
...
#ifdef DTLS_MA_DEBUG
            printf(
                "The buf1 in dtls_ma_cipher_in_str is too small: expected at "
                "least %d, actual %d\n",
...
void dtls_ma_dec(struct dtls_ma_private_key *privk, struct dtls_ma_cipher *D,
                 mpz_t v) {
    // printf("decryption \n");
...
unsigned char *dtls_ma_enc_str_G(struct dtls_ma_public_key *pubk,
    ...
    int group_size = DTLS_MA_G == 0 ? 1 : (DTLS_MA_T_BITS / 8 -
                                           1);  // 1 or (DTLS_MA_T_BITS/8-1)
    ...
    struct dtls_ma_cipher *c =
        malloc(group_size * sizeof(struct dtls_ma_cipher));
    ...
    while (buf_pos < buf_len) {
        ...
        for (i = 0; i < group_size; i++) {
            ...
            int cipher_len = dtls_ma_cipher_out_str(
                c + i, (char *)temp_buf, DTLS_MA_N * DTLS_MA_Q_BITS / 8 + 16);
            if (cipher_len == -1) {
...
unsigned char *dtls_ma_dec_str_G(struct dtls_ma_private_key *privk,
    ...
    while (buf_pos < *buf_len) {
        struct dtls_ma_cipher *temp =
            dtls_ma_cipher_in_str((char *)buf + buf_pos, &pos);
        ...
        // if(mpz_cmp(v0, *(v+0))==0&&mpz_cmp(v1, *(v+1))==0&&mpz_cmp(v2,
        // *(v+2))==0&&mpz_cmp(v3, *(v+3))==0){
        // printf("decoded buf (mode %d) = %s \n", i, mpz_get_str(NULL, 16,
        // *(v+0)));
        // printf("decoded buf (mode %d) = %s \n", i, mpz_get_str(NULL, 16,
        // *(v+1)));
        // printf("decoded buf (mode %d) = %s \n", i, mpz_get_str(NULL, 16,
        ...
        if (group_size > 1) {
            len =
                dtls_ma_decodeG(temp_buf, DTLS_MA_N * DTLS_MA_Q_BITS / 8, v, 0);
        } else
...
int dtls_ma_encodeG(unsigned char *buf, unsigned long buf_len, mpz_t *res) {
    ...
    if (buf_len % group_size != 0) ori_block_num++;
    ...
    while (1) {
        ...
        for (i = 0; i < block_len - 1; i++) {
            block_sign[block_len - 1] =
                block_sign[block_len - 1] ^ block_sign[i];
        }
    ...
    for (k = 0; k < group_size + 1; k++) {
        ...
        for (i = 0; i < temp_return_len; i++) {
            if (i % block_len == 0) {
                ...
                block_sign[block_len - 1] =
                    block_sign[block_len - 1] ^ block_sign[g];
                g++;
            }
        }
        ...
        for (i = 0; i < temp_return_len; i++) {
            ...
            if (index == 0) {
                ...
                if (index == g) {
                    tagged_buf_temp[i] =
                        tagged_buf[i] ^ block_sign[block_len - 1];
                } else {
        ...
        for (g = 0; g < group_size; g++) {
            if (block_sign_check[g] != block_sign[g]) break;
        }
...
int dtls_ma_decodeG(unsigned char *buf, unsigned long buf_len, mpz_t *res,
    ...
    for (i = 0; i < group_size * block_len; i++) {
        ...
        if (index == 0) {
            ...
            for (j = 0; j < block_len; j++) buf_temp[j] = 0x00;
            g++;
        }
    ...
    for (k = 0; k < group_size + 1; k++) {
        ...
        if (num_of_AB >= 1) {
            len_of_last_AA =
                (unsigned char)*(tagged_buf_temp + temp_return_len - 1);
            // len_of_last_AB = (unsigned char)
            // *(tagged_buf_temp+temp_return_len-1);
        ...
        free(buf_temp);
        free(tagged_buf_temp);
        // printf("\nreturn length %d bytes, p=%d, last-AA-len=%d, num_of_AB=%d,
        // num_of_AA=%d, num_of_AB_check=%d\n", return_len, p, len_of_last_AA,
        // num_of_AB, num_of_AA, num_of_AB_check);
        return return_len;
    }
...
int dtls_ma_encode(unsigned char *buf, unsigned long buf_len, mpz_t res) {
    ...
    for (i = 1; i < tagged_buf_len; i++) {
        ...
        block_sign_backup =
            block_sign_backup ^ dtls_ma_common_sign[(c + 1) % 256] ^ c;
    ...
    // printf("(%02x %02x %02x; %02x)\n",tagged_buf[0], tagged_buf[1],
    // tagged_buf[2], block_sign);
        ...
        if (buf_temp[i] == 0x00)
            ...
            for (j = 0; j < count; j++) {
                *(buf + j) = buf_temp[count - 1 - j] ^ block_sign;
                ...
                block_sign_backup =
                    block_sign_backup ^
                    dtls_ma_common_sign[(*(buf + j) + 1) % 256] ^ *(buf + j);
            }
...
unsigned char *dtls_ma_random_hex(int len) {
    ...
    if (res == NULL) dtls_ma_debug("malloc failure!\n");
